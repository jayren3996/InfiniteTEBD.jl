...
@testset "TENSOR_DIAGONAL_MULTIPLICATION" begin
    ...
    left = copy(Γ)
    InfiniteTEBD.tensor_lmul!([2.0, 3.0], left)
    @test left[1, :, :] ≈ 2 .* Γ[1, :, :]
    @test left[2, :, :] ≈ 3 .* Γ[2, :, :]

    right = copy(Γ)
    InfiniteTEBD.tensor_rmul!(right, [5.0, 7.0])
    @test right[:, :, 1] ≈ 5 .* Γ[:, :, 1]
    @test right[:, :, 2] ≈ 7 .* Γ[:, :, 2]
end
...
@testset "TENSOR_GROUPING_INVARIANTS" begin
    ...
    grouped2 = InfiniteTEBD.tensor_group([Γ1, Γ2])
    grouped3 = InfiniteTEBD.tensor_group([Γ1, Γ2, Γ3])

    @test size(grouped2) == (2, 4, 4)
    @test size(grouped3) == (2, 8, 2)
    @test grouped2 ≈ InfiniteTEBD.tensor_group_2(Γ1, Γ2)
    @test grouped3 ≈ InfiniteTEBD.tensor_group_3(Γ1, Γ2, Γ3)

    single = InfiniteTEBD.tensor_group([Γ1])
    @test single == Γ1
    @test single !== Γ1
end
...
@testset "TENSOR_SVD_RECONSTRUCTS_UNTRUNCATED_BLOCK" begin
    ...
    U, S, V = InfiniteTEBD.tensor_svd(T; maxdim=20, svd_min=0.0)
...
@testset "TENSOR_DECOMP_ROUNDTRIPS_GROUPED_BLOCK" begin
    ...
    Γ2 = deterministic_tensor(3, 2, 4)
    Γ3 = deterministic_tensor(4, 2, 2)
    grouped = InfiniteTEBD.tensor_group([Γ1, Γ2, Γ3])

    Γs, λs = InfiniteTEBD.tensor_decomp!(copy(grouped), ones(2), 3; maxdim=20, svd_min=0.0)

    @test length(Γs) == 3
    @test length(λs) == 2
    @test size.(Γs) == [(2, 2, 4), (4, 2, 4), (4, 2, 2)]
    @test InfiniteTEBD.tensor_group(Γs) ≈ grouped atol=1e-10
end
...
@testset "TENSOR_DECOMP_HANDLES_SINGLE_SITE_UNITCELL" begin
    ...
    Γs, λs = InfiniteTEBD.tensor_decomp!(copy(Γ), λ, 1)
    @test length(Γs) == 1
    @test length(λs) == 0
    @test Γs[1] ≈ Γ atol=1e-12

    # return_stats path: bond_stats vector is empty (no internal bonds).
    Γs2, λs2, bond_stats = InfiniteTEBD.tensor_decomp!(copy(Γ), λ, 1; return_stats=true)
    @test length(Γs2) == 1
    @test length(λs2) == 0
    @test isempty(bond_stats)

    # n = 0 is rejected explicitly.
    @test_throws ArgumentError InfiniteTEBD.tensor_decomp!(copy(Γ), λ, 0)
end

@testset "SVD_TRIM_THRESHOLDS_AND_RENORMALIZES" begin
    M = Matrix(Diagonal([4.0, 2.0, 1e-13, 0.0]))

    U, S, V = InfiniteTEBD.svd_trim(M; maxdim=4, svd_min=1e-12)
    @test S ≈ [4.0, 2.0]
    @test U * Diagonal(S) * V ≈ Diagonal([4.0, 2.0, 0.0, 0.0])

    _, S2, _ = InfiniteTEBD.svd_trim(M; maxdim=1, svd_min=0.0, renormalize=true)
    @test length(S2) == 1
    @test norm(S2) ≈ 1.0
end
...
@testset "SVD_TRIM_ITERATIVE_KEEPS_ONE_VALUE_BELOW_THRESHOLD" begin
    ...
    U, S, V = InfiniteTEBD.svd_trim(M; maxdim=3, svd_min=1e-12, use_iterative=true)
...
@testset "SVD_TRUNCATE_BY_ERROR_REJECTS_NONFINITE_INPUT" begin
    ...
    @test_throws ArgumentError InfiniteTEBD._svd_truncate_by_error(M; maxdim=2)
end

@testset "DISCARDED_WEIGHT_SELECTOR_REJECTS_INVALID_ARGUMENTS" begin
    @test_throws ArgumentError InfiniteTEBD._discarded_weight_choice([1.0]; mindim=0)
    @test_throws ArgumentError InfiniteTEBD._discarded_weight_choice([1.0]; maxdim=0)
    @test_throws ArgumentError InfiniteTEBD._discarded_weight_choice([1.0]; mindim=3, maxdim=2)
    @test_throws ArgumentError InfiniteTEBD._discarded_weight_choice([1.0]; truncerr=-1e-3)
    @test_throws ArgumentError InfiniteTEBD._discarded_weight_choice([1.0]; svd_min=-1e-3)
end

@testset "DISCARDED_WEIGHT_SELECTOR_AVOIDS_VECTOR_TEMPORARIES" begin
    s = collect(range(1.0, 0.001; length=1000))
    choice = InfiniteTEBD._discarded_weight_choice(s; mindim=4, maxdim=128, truncerr=1e-8, svd_min=1e-12)
    ...
    f() = InfiniteTEBD._discarded_weight_choice(s; mindim=4, maxdim=128, truncerr=1e-8, svd_min=1e-12)
    f()
...
@testset "APPLY_CHAIN_TRANSFER_MATCHES_GTRM_PRODUCT" begin
    ...
    for (n, χ, d) in [(2, 4, 2), (3, 6, 2), (4, 8, 3), (3, 12, 2)]
        ...
        M = InfiniteTEBD.gtrm(T1s, T2s)
        ρ = randn(ComplexF64, χ, χ)

        ref_r = reshape(M * vec(ρ), χ, χ)
        new_r = InfiniteTEBD.apply_chain_transfer(T1s, T2s, ρ; dir=:r)
        @test new_r ≈ ref_r rtol=1e-10

        ref_l = reshape(transpose(M) * vec(ρ), χ, χ)
        new_l = InfiniteTEBD.apply_chain_transfer(T1s, T2s, ρ; dir=:l)
        @test new_l ≈ ref_l rtol=1e-10
    end
end
...
@testset "APPLY_TRANSFER_REJECTS_BAD_SHAPE_AND_DIR" begin
    ...
    @test_throws DimensionMismatch InfiniteTEBD.apply_transfer(T1, T2, zeros(ComplexF64, 3, 3); dir=:r)
    @test_throws DimensionMismatch InfiniteTEBD.apply_transfer(T1, T2, zeros(ComplexF64, 3, 3); dir=:l)
    @test_throws ArgumentError InfiniteTEBD.apply_transfer(T1, T2, zeros(ComplexF64, 4, 4); dir=:up)
    bad = randn(ComplexF64, 4, 3, 4)  # different physical dim
    @test_throws DimensionMismatch InfiniteTEBD.apply_transfer(T1, bad, zeros(ComplexF64, 4, 4); dir=:r)
end

@testset "APPLY_CHAIN_TRANSFER_REJECTS_BAD_INPUT" begin
    T = [randn(ComplexF64, 4, 2, 4) for _ in 1:3]
    @test_throws ArgumentError InfiniteTEBD.apply_chain_transfer(T, T[1:2], zeros(ComplexF64, 4, 4))
    @test_throws ArgumentError InfiniteTEBD.apply_chain_transfer(typeof(T)(), typeof(T)(), zeros(ComplexF64, 4, 4))
    @test_throws ArgumentError InfiniteTEBD.apply_chain_transfer(T, T, zeros(ComplexF64, 4, 4); dir=:bad)
end
...
@testset "APPLY_TRANSFER_RECTANGULAR_BONDS_AND_FLOAT64" begin
    ...
    for ET in (Float64, ComplexF64)
        for (χL1, χR1, χL2, χR2, d) in [(3, 5, 4, 6, 2), (2, 7, 5, 3, 3), (8, 4, 6, 5, 2)]
            ...
            @test InfiniteTEBD.apply_transfer(T1, T2, ρr; dir=:r) ≈ ref_r rtol=1e-10

            ρl = randn(ET, χL2, χL1)
            ref_l = reshape(transpose(M) * vec(ρl), χR2, χR1)
            @test InfiniteTEBD.apply_transfer(T1, T2, ρl; dir=:l) ≈ ref_l rtol=1e-10
        end
    end
end
...
@testset "CHAIN_TRANSFER_WORKSPACE_REUSES_ACROSS_DIFFERENT_TENSORS" begin
    ...
    ws = InfiniteTEBD.ChainTransferWorkspace(big, big)  # infer T via convenience ctor
    ρ_small = randn(ComplexF64, 10, 10)
    ref = InfiniteTEBD.apply_chain_transfer(small, small, ρ_small; dir=:r)
    @test InfiniteTEBD.apply_chain_transfer!(ws, small, small, ρ_small; dir=:r) ≈ ref rtol=1e-10
end
...
@testset "APPLY_CHAIN_TRANSFER_INPLACE_ALLOCATIONS_BOUNDED" begin
    ...
    T2s = [randn(ComplexF64, χ, d, χ) for _ in 1:n]
    ρ = randn(ComplexF64, χ, χ)
    ws = InfiniteTEBD.ChainTransferWorkspace(T1s, T2s)
    InfiniteTEBD.apply_chain_transfer!(ws, T1s, T2s, ρ; dir=:r)  # warmup
    InfiniteTEBD.apply_chain_transfer!(ws, T1s, T2s, ρ; dir=:r)
    # Budget covers per-site reshape SubArray wrappers (n*~80 B) plus a little slack.
    @test (@allocated InfiniteTEBD.apply_chain_transfer!(ws, T1s, T2s, ρ; dir=:r)) < 4_000
end
