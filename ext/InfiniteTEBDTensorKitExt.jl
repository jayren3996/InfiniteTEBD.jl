...
The provided spaces must admit a nonzero state: if the site hom-space
`vspace ⊗ pspace ← vspace` has dimension zero (charge frustration — e.g. a
spin-1/2 physical space with charges ±1 against a bond space carrying only
even charges), the only tensor with these spaces is the zero tensor, and the
constructor throws an `ArgumentError` instead of silently returning the zero
state.
...
function rand_iMPS(pspace::VectorSpace, vspace::VectorSpace, n::Integer)
    ...
    return _rand_symmetric_iMPS(pspace, fill(vspace, Int(n)))
end

# Internal: shared random-state builder with per-bond virtual spaces.
# `vspaces[i]` is the bond space to the RIGHT of site i (matching `ψ.λ[i]`),
# so site i's left bond is `vspaces[mod1(i-1, n)]` with periodic wraparound:
# bond n doubles as the left bond of site 1. Site hom-spaces are validated by
# the `iMPS` inner constructor (`_validate_iMPS_bonds`), which rejects
# zero-dimensional hom-spaces with a charge-frustration diagnostic.
function _rand_symmetric_iMPS(pspace::VectorSpace,
                              vspaces::AbstractVector{<:VectorSpace})
    n = length(vspaces)
    n > 0 || throw(ArgumentError("n must be positive (got $n)"))
    Γ = [randn(ComplexF64, vspaces[mod1(i - 1, n)] ⊗ pspace ← vspaces[i])
         for i in 1:n]
    λ = [DiagonalTensorMap(ones(Float64, dim(vspaces[i])), vspaces[i])
         for i in 1:n]
    return iMPS(Γ, λ, n)
end
...
# Internal: integer group operations for the Abelian symmetries supported by
# the symbol-based constructor. U(1) charges live in ℤ, Z₂ charges in ℤ/2ℤ.
_charge_add(::Val{:U1}, a::Integer, b::Integer) = Int(a) + Int(b)
_charge_neg(::Val{:U1}, a::Integer)             = -Int(a)
_charge_add(::Val{:Z2}, a::Integer, b::Integer) = mod(Int(a) + Int(b), 2)
_charge_neg(::Val{:Z2}, a::Integer)             = mod(-Int(a), 2)

# Internal: deterministic single-lap closed charge walk 0 → … → 0 around the
# unit cell, picking one physical charge per site. `R[k+1]` must be the set of
# cumulative charges reachable after k sites (seeded at 0). Returns `loop` of
# length n+1 with loop[1] = loop[n+1] = 0; loop[k+1] is the anchor charge of
# bond k, used to weight/trim sector multiplicities. Callers must ensure a
# closed loop exists (0 ∈ R[n+1]) before calling.
function _canonical_charge_loop(v::Val, pcharges::Vector{Int},
                                R::Vector{Set{Int}}, n::Integer)
    loop = Vector{Int}(undef, n + 1)
    loop[n + 1] = 0
    for k in n:-1:1
        best = nothing
        for c in pcharges
            prev = _charge_add(v, loop[k + 1], _charge_neg(v, c))
            if prev in R[k] && (best === nothing || abs(prev) < abs(best))
                best = prev
            end
        end
        best === nothing && throw(ArgumentError(
            "_canonical_charge_loop: no closed charge loop through bond $k " *
            "(internal error — feasibility should have been checked upstream)"))
        loop[k] = best
    end
    return loop
end
...
"""
    _auto_bond_spaces(sym, pspace, χ, n; flux=0) -> Vector of bond spaces

Build one graded bond space per bond of the unit cell — `out[k]` is the bond
to the RIGHT of site `k` (matching `ψ.λ[k]`); bond `n` is the wraparound bond
and doubles as the left bond of site 1 — such that every site hom-space
`V_{k-1} ⊗ P ← V_k` has nonzero dimension, including the periodic seam.

Construction: cumulative charge fusion around the unit cell (the set-valued
generalisation of `product_iMPS`'s cumulative-charge bond decoration). The
wraparound bond is seeded with every single-lap charge total `t` whose inverse
`-t` is also a single-lap total (so a two-lap closed loop passes through `t`);
the seed is propagated forwards and backwards through the cell, and bond `k`
keeps the sectors that are both reachable from the seed and able to close back
into it. Every kept sector therefore lies on at least one closed charge loop.
For spin-1/2 U(1) charges ±1 this makes consecutive bonds alternate parity
(one bond carries odd charges, the next even). Each bond then receives total
dimension ≈ χ, distributed uniformly across its sectors (the remainder goes to
the sectors nearest a canonical closed loop; if a bond has more candidate
sectors than χ, only the χ nearest the loop are kept).

Throws an `ArgumentError` when the symmetry is charge-frustrated on this unit
cell: no choice of one physical charge per site sums to zero around the cell,
so no flux-0 bond decoration can close and every site tensor would be
identically zero. Example: spin-1/2 U(1) charges ±1 with odd `n` (any such
cell carries a half-odd-integer net charge, so `n=1` is impossible).
"""
function _auto_bond_spaces(sym::Symbol, pspace::VectorSpace, χ::Integer,
                           n::Integer; flux::Integer=0)
    χi = Int(χ)
    ni = Int(n)
    # :Trivial has no grading and therefore no frustration: every bond is the
    # plain χ-dimensional space.
    sym === :Trivial && return [ComplexSpace(χi) for _ in 1:ni]

    v = Val(sym)
    pcharges = sort!(unique(Int[_sector_int(s) for s in sectors(pspace)]))

    # R[k+1]: cumulative charges reachable after k sites, seeded at 0.
    R = Vector{Set{Int}}(undef, ni + 1)
    R[1] = Set{Int}((0,))
    for k in 1:ni
        R[k + 1] = Set{Int}(_charge_add(v, a, c) for a in R[k] for c in pcharges)
    end

    # Wraparound feasibility: some assignment of one physical charge per site
    # must sum to zero around the cell.
    0 in R[ni + 1] || throw(ArgumentError(
        "rand_iMPS: charge-frustrated unit cell — no choice of one physical " *
        "charge per site (physical charges $(pcharges), symmetry :$sym) sums " *
        "to zero around an n=$ni unit cell, so no flux-0 bond decoration " *
        "closes the cell and every site tensor would be identically zero. " *
        "Example: spin-1/2 U(1) charges ±1 admit no symmetric iMPS on an odd " *
        "unit cell (the cell's net charge is half-odd-integer). Fixes: pick " *
        "a compatible unit-cell length (e.g. even n for charges ±1), or " *
        "construct the state from explicit bond spaces via " *
        "`rand_iMPS(pspace, vspace, n)`."))

    # Seed sectors on the wraparound bond: single-lap totals t whose inverse
    # is also a single-lap total, so a two-lap closed loop passes through t.
    # This enriches the wraparound bond beyond {0} while keeping every sector
    # on a closed charge loop. 0 itself always qualifies (feasibility above).
    seed = Set{Int}(t for t in R[ni + 1] if _charge_neg(v, t) in R[ni + 1])

    # Forward (from the seed) and backward (into the seed) reachable sets.
    F = Vector{Set{Int}}(undef, ni + 1)
    F[1] = seed
    for k in 1:ni
        F[k + 1] = Set{Int}(_charge_add(v, a, c) for a in F[k] for c in pcharges)
    end
    B = Vector{Set{Int}}(undef, ni + 1)
    B[ni + 1] = seed
    for k in ni:-1:1
        B[k] = Set{Int}(_charge_add(v, b, _charge_neg(v, c))
                        for b in B[k + 1] for c in pcharges)
    end

    loop = _canonical_charge_loop(v, pcharges, R, ni)

    return map(1:ni) do k
        # Sectors on bond k: reachable from the seed AND able to close back
        # into it. Never empty: the canonical loop's bond-k charge qualifies.
        cands = sort!(collect(intersect(F[k + 1], B[k + 1])))
        isempty(cands) && throw(ArgumentError(
            "_auto_bond_spaces: no closable sector on bond $k (internal " *
            "error — feasibility should have guaranteed at least one)"))
        anchor = loop[k + 1]
        by_loop_distance = s -> (abs(s - anchor), abs(s), s)
        if length(cands) > χi
            order = sortperm(cands; by=by_loop_distance)
            cands = sort!(cands[order[1:χi]])
        end
        m = length(cands)
        per = χi ÷ m                       # ≥ 1 because m ≤ χ after trimming
        dims = fill(per, m)
        order = sortperm(cands; by=by_loop_distance)
        for idx in order[1:(χi - per * m)] # distribute the remainder
            dims[idx] += 1
        end
        graded_space(sym, [cands[j] => dims[j] for j in 1:m]...)
    end
end
...
The bond spaces are built per bond by cumulative charge fusion around the
unit cell (see `_auto_bond_spaces`), so consecutive bonds always connect
through one physical charge and the constraint closes around the periodic
seam. For spin-1/2 U(1) charges ±1 this means the bonds alternate parity
(odd-charge bond, even-charge bond, …) and the resulting tensors are nonzero.
If the charges are frustrated on the requested cell — no choice of one charge
per site sums to zero, e.g. `rand_iMPS(:U1, [-1, 1]; χ=8, n=1)` (or any odd
`n`) — an `ArgumentError` explains the frustration instead of silently
returning a zero state.
...
function rand_iMPS(sym::Symbol, charges::AbstractVector{<:Integer};
    ...
    Vs = _auto_bond_spaces(sym, P, χ, n; flux=flux)
    return _rand_symmetric_iMPS(P, Vs)
end
...
function _validate_iMPS_bonds(
    ...
    # Structural zero-state check (after the bond-matching checks above so
    # mismatched spaces keep reporting DimensionMismatch): a site whose
    # hom-space `V_left ⊗ P ← V_right` has dimension zero admits ONLY the zero
    # tensor — the state is identically zero regardless of the stored values,
    # and canonical!/evolve!/expect/ent_S would all silently return 0.0.
    # This happens when the bond gradings are charge-frustrated: no sector of
    # the left bond fuses with a physical charge into a sector of the right
    # bond. Reject loudly instead of constructing a dead state.
    for i in 1:n
        hom = space(Γ[i])
        dim(hom) > 0 || throw(ArgumentError(
            "SymmetricIMPS site $i: the site hom-space $hom has dimension " *
            "zero, so every tensor with these spaces is identically zero and " *
            "the state is the zero state (all observables would silently " *
            "evaluate to 0.0). The bond gradings are charge-frustrated: no " *
            "sector of the left bond fuses with a physical-leg charge into a " *
            "sector of the right bond (the constraint must also close around " *
            "the periodic unit cell). Fixes: use a unit-cell length n " *
            "compatible with the physical charges (e.g. spin-1/2 U(1) " *
            "charges ±1 need even n; n=1 is impossible), let " *
            "`rand_iMPS(sym, charges; χ, n)` auto-build compatible bond " *
            "spaces, or supply explicit bond spaces in which consecutive " *
            "bonds differ by one physical charge."))
    end
    return nothing
end
...
`P ⊗ P` and domain `P ⊗ P`, where `P` matches the physical leg of `ψ`. The
unit cell must contain at least two sites (`ψ.n ≥ 2`): on a single-site cell
the two gate legs would alias the same tensor, so the call throws an
`ArgumentError` and leaves the state untouched.
...
function applygate!(ψ::iMPS{<:AbstractTensorMap, <:DiagonalTensorMap},
    ...
    # Two-site gates need two distinct sites. On an n=1 unit cell the periodic
    # normalization maps (i, j) = (1, 2) to (1, 1), the nearest-neighbour
    # validator below would accept it (mod1(2, 1) == 1), and the two halves of
    # the SVD would then be written to the SAME ψ.Γ slot — the second
    # assignment clobbering the first and silently corrupting the state (the
    # dense backend's tensor_decomp! guards the equivalent case). Reject up
    # front, before the state is touched.
    ψ.n ≥ 2 || throw(ArgumentError(
        "v1 symmetric applygate! requires a unit cell with at least 2 sites " *
        "for a two-site gate (got ψ.n = $(ψ.n)): on an n=1 cell sites i and " *
        "j alias the same tensor, so the update would silently corrupt the " *
        "state. Rebuild the state with n ≥ 2 (e.g. repeat the single-site " *
        "unit cell) to apply two-site gates."))
